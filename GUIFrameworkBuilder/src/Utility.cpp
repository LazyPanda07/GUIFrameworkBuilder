...
void addProjectsToSlnFile(const json::JSONParser& buildSettings, string& slnFile)
{
	...
	for (const auto& i : dependencies->data)
	{
		...
		{
			addProject = "Project(" + sln::visualCPlusPlusProjectGUID + ") = " + addQuotes(i.first) + ", " + addQuotes(guiFrameworkFolder + '\\' + i.first + '\\' + i.first + extensions::projFile) + ", " + addQuotes(get<string>(i.second)) +
				'\n' + sln::endProject + '\n';
		}
		...
		addConfigurationPlatforms(startProjectConfigurationPlatforms, get<string>(i.second));
	}
...
void addAdditionalIncludeDirectories(const json::JSONParser& buildSettings, string& vcxprojFile)
{
	...
	for (const auto& i : dependencies->data)
	{
		...
		{
			additionalIncludeDirectories += vsMacros::solutionDir + guiFrameworkFolder + "\\" + it->second + ';';
		}
		else
		{
			additionalIncludeDirectories += vsMacros::solutionDir + guiFrameworkFolder + "\\" + i.first + "\\src;";
		}
	}
...
void addProjectReference(const json::JSONParser& buildSettings, string& vcxprojFile)
{
	...
	string projectReference = topLevelSpaces + vcxproj::startItemGroupTag + '\n' +
		oneBottomLevelSpaces + "<ProjectReference Include=\"..\\" + guiFrameworkFolder + '\\' + guiFrameworkName + '\\' + guiFrameworkName + extensions::projFile + "\">\n" +
		twoBottomLevelSpaces + vcxproj::startProjectTag + guiFrameworkGUID + vcxproj::endProjectTag + '\n' +
